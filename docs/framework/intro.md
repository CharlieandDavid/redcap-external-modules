...
### Introduction to Modules Framework Versioning
...
convertIntsToStrings($row)) | `?.?.?`<br>`3` | Returns a copy of the specified array with any integer values cast to strings.  This method is intended to aid in converting queries to use parameters with minimal refactoring.
...
createProject($title, $purpose, [, $project_note])) | `?.?.?`<br>`3` | Creates a new redcap project and returns the project id.
...
getEventId()) | `?.?.?`<br>`3` | Returns the current event ID.  If an 'event_id' GET parameter is specified, it will be returned.  If not, and the project only has a single event, that event's ID will be returned.  If no 'event_id' GET parameter is specified and the project has multiple events, an exception will be thrown.
...
importDataDictionary($project_id,$path)) | `?.?.?`<br>`3` | Given a project id and a path, imports a data dictionary CSV file.
...
isPage($path)) | `?.?.?`<br>`3` | Returns true if the current page matches the supplied file/dir path.  The path can be any file/dir under the versioned REDCap directory (ex: `Design/online_designer.php`).
