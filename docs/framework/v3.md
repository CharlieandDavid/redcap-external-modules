...
getSelectElementEnumLabel($project_id,$field_name) | Given a project id and a field name, it will return an array of values and labels. It only works for select types.
...
importDataDictionary($project_id,$path) | Given a project id and a path, imports a data dictionary CSV file. The path has to be located inside the module.
