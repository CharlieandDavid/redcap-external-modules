...
getSafePath($path[, $root]) | Checks a file path to make sure a [path traversal attack](https://www.owasp.org/index.php/Path_Traversal) is not in progress and returns a normalized path similar to PHP's `realpath()` function.  If a path traversal attack is detected, an exception is thrown.  This is very import when generating paths using strings created from user input.  The `$path` is expected to be relative to the `$root`.  If `$root` is not specified, the module directory is assumed.  The `$root` can be either absolute or relative to the module directory.  A path traversal attack is considered to be in progress if the the `$root` does not contain the `$path`.
